...
if [[ $MAIN_OPTION -eq 1 ]]; then
    ...
    if [[ $DEVICE_TYPE -eq 1 ]]; then
        ...
        if [ -s raw_$PAYLOAD_NAME ]; then
            ...
            cp $PAYLOAD_NAME "$PAYLOADS_DIR/"
            echo "[+] Payload copied to Apache server at: http://$LHOST/$PAYLOAD_NAME"
            echo "[+] Payload also saved in the script's folder under 'payloads/$PAYLOAD_NAME'"
        else
